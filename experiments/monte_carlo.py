...
- Introducir incertidumbre en:
  ...
  - Duración de los cursos (perturbación ±20% vía distribución triangular).
...
# Directorios de salida de resultados y figuras.
...
def triangular_tti(a: float, b: float, c: float) -> float:
    """
    Genera una variable aleatoria con distribución triangular mediante
    la Transformada de la Inversa (TTI).

    Parámetros
    ----------
    a : float
        Valor mínimo de la distribución.
    b : float
        Valor máximo de la distribución.
    c : float
        Moda (valor más probable), con a ≤ c ≤ b.
...
def perturbar_duracion(duracion_original: int, factor: float = 0.20) -> int:
    """
    Aplica una perturbación triangular a la duración nominal de un curso.

    El modelo asume que la duración real puede variar entre:
    - (1 - factor) veces la duración original
    - (1 + factor) veces la duración original

    con mayor probabilidad en el valor nominal (moda).

    Devuelve la duración perturbada redondeada y acotada a un mínimo de 1 semana.
    """
...
class _GrafoPerturbado:
    """
    Proxy de GrafoCursos que conserva la estructura del grafo original
    pero reemplaza las duraciones por versiones perturbadas.

    Este adaptador permite evaluar el efecto de la incertidumbre sin modificar
    el grafo base del proyecto.
    """
    ...
    def __init__(self, grafo: GrafoCursos, factor: float = 0.20):
        ...
        # Se crea una copia curso por curso, perturbando solo la duración.
    ...
    def aplicar_curso(self, hab, curso):
        """Aplica la transición de estado al tomar un curso."""
        return hab | curso.habilidades

    def es_objetivo(self, hab, perfil_id):
        """Comprueba si el estado satisface el perfil objetivo."""
        return self.perfiles[perfil_id].habilidades_requeridas.issubset(hab)

    def habilidades_faltantes(self, hab, perfil_id):
        """Devuelve las habilidades del perfil objetivo que aún faltan."""
        return self.perfiles[perfil_id].habilidades_requeridas - hab


# *** Generación de instancias aleatorias *******

def generar_instancia_aleatoria(grafo: GrafoCursos, max_habs_iniciales: int = 5,) -> tuple[frozenset, str]:
    """
    Genera una instancia aleatoria para una corrida Monte Carlo.

    La instancia incluye:
    - un perfil objetivo elegido uniformemente al azar;
    - un subconjunto aleatorio de habilidades iniciales.

    La generación evita, en la medida de lo posible, que el estado inicial
    ya satisfaga completamente el perfil objetivo.
    """
    ...
    # Asegurar que la instancia no nazca ya resuelta.
...
class ResultadoMC:
    """
    Resultado de una corrida individual de la simulación Monte Carlo.
    """
    corrida: int
    perfil_id: str
...
def ejecutar_simulacion(grafo: GrafoCursos, n_runs: int = 500, factor_perturb: float = 0.20, max_habs_ini: int = 5,) -> list[ResultadoMC]:
    """
    Ejecuta n_runs corridas Monte Carlo independientes.

    En cada corrida:
    - se genera una instancia aleatoria;
    - se construye un grafo perturbado;
    - se ejecutan A* y Greedy;
    - se almacenan métricas de desempeño y coste.
    """
    ...
    for i in range(1, n_runs + 1):
        ...
        for alg_fn, alg_nombre in [(astar, "A*"), (greedy, "Greedy")]:
            ...
            sem_nom = (
                sum(grafo.cursos[c.id].duracion_semanas for c in r.trayectoria)
                if r.exito else 0
            )
            ...
            if r.exito:
                if alg_nombre == "A*":
                    exitos_a += 1
                if alg_nombre == "Greedy":
                    exitos_g += 1

        if i % 50 == 0 or i == n_runs:
            elapsed = time.perf_counter() - t0
            print(
                f"  [{i / n_runs * 100:5.1f}%] Corrida {i:>4}/{n_runs}  "
...
def ic_95(valores: list[float]) -> tuple[float, float]:
    ...
    Se usa z = 1.96, una aproximación razonable cuando n es suficientemente
    grande (en este proyecto se considera válida para n ≥ 30).
    """
    if len(valores) < 2:
        return (0.0, 0.0)
    n = len(valores)
...
def analizar(resultados: list[ResultadoMC]) -> dict:
    """
    Calcula estadísticas descriptivas por algoritmo a partir de las corridas.
    """
    stats = {}
    for alg in ["A*", "Greedy"]:
...
def imprimir_estadisticas(stats: dict, n_runs: int) -> None:
    """
    Imprime un resumen estadístico legible de los resultados Monte Carlo.
    """
    print(f"\n{SEP}")
...
def generar_graficos(stats: dict, resultados: list[ResultadoMC]) -> None:
    """
    Genera las figuras de análisis Monte Carlo.

    Se construyen tres gráficos:
    - histogramas comparativos,
    - comparativa con barras e intervalos de confianza,
    - dispersión de perturbación nominal vs perturbada.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
    ...
    def _estilo(fig, axes):
        """Aplica un estilo oscuro consistente a las figuras."""
    ...
    ax.set_title(
        "Efecto de la perturbación triangular ±20%\n"
...
def main() -> None:
    """
    Punto de entrada de la simulación Monte Carlo.
    """
