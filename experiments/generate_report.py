...
def resumen_algoritmo(rows: list[dict], predicado) -> dict:
    ...
    if not seleccion:
        return {k: "—" for k in
                ("n", "exitos", "exito_rate", "cursos_prom", "semanas_prom",
                 "nodos_prom", "tiempo_prom", "validas")}
...
def build_report() -> str:
    ...
    tabla_principal = md_table(
        ["Algoritmo", "Instancias", "Éxito", "Cursos prom.",
         "Semanas prom.", "Nodos prom.", "Tiempo prom. (s)", "Tray. válidas"],
        [
            ["A*",     str(astar["n"]),  astar["exito_rate"],
             astar["cursos_prom"],  astar["semanas_prom"],
             astar["nodos_prom"],   astar["tiempo_prom"],  astar["validas"]],
            ["Greedy", str(greedy["n"]), greedy["exito_rate"],
             greedy["cursos_prom"], greedy["semanas_prom"],
             greedy["nodos_prom"],  greedy["tiempo_prom"], greedy["validas"]],
        ],
    )

    # Delta promedio de semanas A* vs Greedy
    delta_txt = ""
    try:
        a_dict = {r["instancia_id"]: r for r in sin_llm
                  if "A*" in r.get("algoritmo", "") and _to_bool(r.get("exito"))}
        g_dict = {r["instancia_id"]: r for r in sin_llm
    ...
    if llm is None:
        ...
        if (llm["long_n"] > 0 and llm["short_n"] > 0
    ...
    report = [
        ...
        f"_Generado automáticamente el "
