...
import matplotlib
matplotlib.use("Agg")  # Backend no interactivo para guardar imágenes en disco.
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import numpy as np

# Directorios de resultados y salida de figuras.
...
# Colores base para comparar algoritmos.
C_ASTAR = "#2563EB"
C_GREEDY = "#F59E0B"

# Colores asociados a cada perfil objetivo.
PERFIL_COLOR = {
...
# Etiquetas legibles para la leyenda de perfiles.
PERFIL_LABEL = {
...
# Color por defecto cuando un perfil no está en el mapeo anterior.
_COLOR_DEFAULT = "#64748B"


def _color_perfil(perfil: str) -> str:
    """Devuelve el color asociado a un perfil objetivo."""
...
def leer_csv(nombre: str) -> list[dict]:
    """
    Lee un CSV dentro de la carpeta results y devuelve sus filas como diccionarios.

    Si el archivo no existe, se retorna una lista vacía para que el flujo
    del script pueda continuar sin interrumpirse.
    """
...
def _safe_int(valor, default: int = 0) -> int:
    """Convierte valores provenientes del CSV a entero de forma tolerante."""
    try:
...
def _safe_float(valor, default: float = np.nan) -> float:
    """Convierte valores provenientes del CSV a flotante de forma tolerante."""
    try:
...
def grafico_nodos(sin_llm: list[dict]) -> None:
    """
    Genera un gráfico de barras comparando nodos expandidos entre A* y Greedy.

    La escala logarítmica ayuda a visualizar diferencias grandes entre ambos
    algoritmos cuando el número de nodos varía mucho entre instancias.
    """
    astar_dict = {
        r["instancia_id"]: r
        for r in sin_llm
    ...
    # Barras comparativas: A* y Greedy.
    ...
    # Fondo suave por perfil para distinguir grupos visualmente.
    for i, pid in enumerate(perfiles):
        ax.axvspan(i - 0.5, i + 0.5, alpha=0.05, color=_color_perfil(pid), zorder=0)

    # Etiqueta de razón A*/Greedy sobre cada pareja de barras.
    ...
    ax.legend(
        handles=[
            mpatches.Patch(color=v, label=PERFIL_LABEL.get(k, k))
...
def _score_style(score: float) -> tuple[str, str]:
    """Asigna una etiqueta cualitativa y un color según la puntuación."""
    if score >= 9:
...
def grafico_llm(con_llm: list[dict]) -> None:
    """
    Genera un gráfico de puntuaciones LLM por instancia.

    El gráfico destaca visualmente las trayectorias largas y permite observar
    la relación entre la extensión de la solución y la nota otorgada por el LLM.
    """
    filas = []
    for r in con_llm:
    ...
    # Zonas de calidad por rangos de puntuación.
    ...
    # Cada punto resalta si la trayectoria es larga.
    for i, f in enumerate(filas):
        marker = "X" if f["larga"] else "o"
        ax.scatter(
            i,
            f["p"],
            s=105 if f["larga"] else 82,
            marker=marker,
            color=face_colors[i],
            edgecolor=_color_perfil(f["perfil"]),
            linewidth=1.8,
            zorder=4,
        )
        ax.text(
...
def grafico_calidad(sin_llm: list[dict]) -> None:
    """
    Genera un gráfico de dos paneles con cursos y semanas para la solución A*.

    Permite comparar la calidad estructural de la trayectoria por instancia,
    mostrando simultáneamente el número de cursos y la duración total.
    """
    astar_rows = [
        r for r in sin_llm
        if "A*" in r.get("algoritmo", "") and r.get("exito") == "True"
    ...
    # Panel superior: número de cursos.
...
def grafico_tiempo(sin_llm: list[dict]) -> None:
    """
    Genera un gráfico comparando el tiempo de búsqueda de A* y Greedy.

    El eje Y está en escala logarítmica para poder observar diferencias
    significativas incluso cuando los tiempos son muy pequeños.
    """
    astar_dict = {
        r["instancia_id"]: r
        for r in sin_llm
...
def main() -> None:
    """Carga los CSV experimentales y genera las cuatro figuras del análisis."""
    print("=" * 56)
